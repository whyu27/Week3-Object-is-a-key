...
const items = [
  ...
  if(money < 50000){
    console.log('Mohon maaf, uang anda tidak cukup');
    return;
  }
   ...
   for(let i=0; i<items.length; i++){
    let namaBarang = items[i][0];
    let hargaBarang = items[i][1];
    if(money >= items[i][1]){
        obj.listPurchased.push(namaBarang);
        money = money - hargaBarang;            // uang berkurang beneran
        obj.changeMoney = money; 
    }
   }
   return obj;
